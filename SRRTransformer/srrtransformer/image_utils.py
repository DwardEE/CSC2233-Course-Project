        ...
        for d in range(patch_size[0]):
            ...
            if d % 2 == 0:
                ...
                for i in range(d+1):
                    ...
                    answer[String_size-i, String_size -
                           j] = val_list.index(Dna_string[Total_size - sum - i])+min_value
                    if j == 0:
                        break
                    j -= 1
            if d % 2 == 1:
                ...
                for j in range(d+1):
                    ...
                    answer[String_size-i, String_size -
                           j] = val_list.index(Dna_string[Total_size - sum - j])+min_value
                    if i == 0:
                        break
                    i -= 1
        ...
        for d in range(patch_size[0]):
            ...
            if d % 2 == 0:
                ...
                for i in range(d+1):
                    ...
                    answer[String_size-i, String_size -
                           j] = val_list.index(Dna_string[Total_size - sum - i])
                    if j == 0:
                        break
                    j -= 1
            if d % 2 == 1:
                ...
                for j in range(d+1):
                    ...
                    answer[String_size-i, String_size -
                           j] = val_list.index(Dna_string[Total_size - sum - j])
                    if i == 0:
                        break
                    i -= 1
    return answer
    ...
    *Converts a Block matrix to a list in which every instance is  a list representing the DNA encoding the each block of the matrix* 
    ...
    for i in range(matrix_shape[0]):
        for j in range(matrix_shape[1]):
            dna_patch = convert_gray_image_to_dna_diag_normalized(
                B_matrix[i, j], DNA_dictionary)
...
def File2DNA(file_name=None, folder_name=None, suffix=None, limit_doc_num=None):
    ...
    *Create a data frame of the DNA encoding of an image or a set of images* 
    ...
    if file_name and folder_name:
        print(
            "Please choose file_name (for single file) OR folder_name (for multiple files)!")
    if file_name:
    ...
    print("There are " + str(len(np_one_channel_image_list)/3) +
    ...
    if Normalized == True:
        for i in tqdm(range(len(one_channel_image_dna_list))):
            ...
            for j in range(len(img)):
                ...
                DNA_len = len(DNA_str)
                doc_list.append({"DNA": DNA_str, "DNA_len": DNA_len, "doc_id": doc_id, "color_id": color_id,
                                "chunk_id": chunk_id, "DNA_min_value": DNA_min_value, "orig_image_size": image_size})
                chunk_id += 1
...
def convert_from_dna_to_block_matrix(dna_list, image_size, length_of_codewords=k_mer_size):
    ...
    *Create a block matrix from dna list* 
    ...
    """
    block_matrix_size = (int(image_size[0]/patch_size[0]), int(
        image_size[1]/patch_size[1]), int(patch_size[0]), int(patch_size[1]))
    ...
    for i in range(block_matrix_size[0]):
        for j in range(block_matrix_size[1]):
            ...
            dna_list_to_convert_split_to_n = [dna_list_to_convert[i:i+length_of_codewords]
                                              for i in range(0, len(dna_list_to_convert), length_of_codewords)]
            answer[i, j] = convert_dna_to_gray_image_diag_normalized(
                dna_list_to_convert_split_to_n)
    return answer
...
def JoinChunnles(dna_list, image_size):
    ...
    *Construct a color image from dna list* 
    ...
    image_matrix[:, :, 0] = convert_from_block_matrix_to_image(
        convert_from_dna_to_block_matrix(im_array_R, image_size), image_size)
    image_matrix[:, :, 1] = convert_from_block_matrix_to_image(
        convert_from_dna_to_block_matrix(im_array_G, image_size), image_size)
    image_matrix[:, :, 2] = convert_from_block_matrix_to_image(
        convert_from_dna_to_block_matrix(im_array_B, image_size), image_size)
    return image_matrix
...
def ImageReconstruction(DNA_column=None, min_value_column=None, image_size_column=None):
    ...
    *Construct a color image* 
    ...
    if Normalized == True:
        ...
        joint_list = [seq2splits(dna_str.replace(" ", ''), k_mer_size).split(
