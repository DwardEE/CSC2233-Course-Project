...
def TrainTestSplitByDocs(error_df, eval_and_test=True,
    ...
    *Splits data frame to train and test* 
    ...
    if eval_and_test:
        ...
        train_docs, test_eval_docs = train_test_split(
            doc_ids, test_size=train_eval_ratio, random_state=random_state)
        eval_docs, test_docs = train_test_split(
            test_eval_docs, test_size=test_eval_ratio, random_state=random_state)
        ...
        train_docs, eval_docs = train_test_split(
            doc_ids, test_size=eval_ratio, random_state=random_state)
        train_df = error_df.loc[error_df[doc_column_name].isin(train_docs)]
        eval_df = error_df.loc[error_df[doc_column_name].isin(eval_docs)]
        data_splits = [train_df, eval_df]
    if do_print:
        print("Length:")
        print(f"Full data: {len(error_df)}", [
              f"{data_names[i]}: {len(data_splits[i])}" for i in range(len(data_splits))])
        if HammingScore_column:
            print("Mean HammingScore:")
            print(f"Full data: {error_df[HammingScore_column].mean()}", [
                  f"{data_names[i]}: {round(data_splits[i][HammingScore_column].mean(),6)}" for i in range(len(data_splits))])
    return data_splits
...
def Data2Dataset(*dfs, errored_column_name="dna_copy", label_column_name="orig_DNA", make_pre_train=False):
    ...
    *Create a dataset for training* 
    ...
    error_df["kmer_W_errors"] = [
        seq2kmer(seq, 4) for seq in error_df[errored_column_name]]
    error_df["split_orig"] = [seq2splits(seq, 4)
                              for seq in error_df[label_column_name]]
