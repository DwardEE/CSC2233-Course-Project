...
def SplitCL2Realible(df):
    ...
    *Splits Correct Length sequences to realible (with only valid codewords) and not_realiable (with non valid codewords)
    ...
    k_mer_size = config["k_mer_size"]
    seq_length = config["seq_length"]
    codewords = [''.join(i) for i in product(
        ["A", "C", "T", "G"], repeat=k_mer_size)]
    data['splited_orig'] = [seq2splits(seq, k_mer_size)
                            for seq in data["orig_DNA"]]
    ...
    data['splited_copy'] = [seq2splits(seq, k_mer_size)
                            for seq in data["dna_copy"]]

    realible_length_reads = data.loc[data.dna_copy_len == seq_length].copy()

    realible_data = realible_length_reads.loc[~(
        realible_length_reads.splited_copy.str.contains(error_codewords_str))].copy()
    not_realible_data = realible_length_reads.loc[realible_length_reads.splited_copy.str.contains(
        error_codewords_str)].copy()
...
def Realible2UnrealibleCodewords(realible_codewords):
    ...
    *Given a list of realiable codewords, returns the unrealible*
    ...
    """
    all_possible_codewords = [''.join(i) for i in product(
        ["A", "C", "T", "G"], repeat=k_mer_size)]
    error_codewords = [
        i for i in all_possible_codewords if i not in realible_codewords]
...
def GetRealibleData(df, realible_codewords):
    ...
    *Given a df and a list of realible codewords used to encode the data in the df, return the realiable and not realiable data,
    ...
    realible_length_reads = data.loc[data["dna_copy_len"] == seq_length].copy()
    realible_length_reads['splited_copy'] = [seq2splits(
        seq, k_mer_size) for seq in realible_length_reads["dna_copy"]]
    realible_data = realible_length_reads.loc[~(
        realible_length_reads["splited_copy"].str.contains(error_codewords_str))].copy()
    not_realible_data = realible_length_reads.loc[realible_length_reads["splited_copy"].str.contains(
        error_codewords_str)].copy()
...
# Function that takes 2 strings and returns the detailed num of operations to get from x to y
...
def SplitReadsByType(df, realible_codewords=None):
    ...
    *Splits a data frame to 
    ...
    predictions_sets["GTGS"], predictions_sets["CL"], error_codewords = GetRealibleData(
        df, realible_codewords)
...
def MakeGTGSResults(error_df):
    ...
    *Predict GTGS with identity mapping and calculate error parameters   *
    ...
    pred_hamming_distance = batch_normalized_hamming_distance(
        pred_str, label_str, return_list=True)
    ...
    error_df['pred_is_error'] = (
        error_df['dna_recoverd'] != error_df["orig_DNA"]).astype(int)
...
def PredictEndDecode(error_df, model_dir, error_codewords=None, batch_size=128):
    ...
    *Predicts and decodes error strings in a data frame*
    ...
    dna_model = DNAmodel.DnaModel(
        batch_size=batch_size, from_pretrained=True, model_dir=model_dir)
    dna_model.load_model()

    bad_words_id = None
    if error_codewords:
        print("using error codewords...")
        bad_words_id = dna_model.tokenizer(
            error_codewords, add_special_tokens=False).input_ids
    ...
    for i in tqdm(idx):
        ...
        prediction = dna_model.tokenizer.batch_decode(
    ...
    print(
        f"HammingScore: {HammingScore*100} %   Edit_error_rate:{error_rate*100} %")
    metrics = {"HammingScore": HammingScore,
               "ErrorScore": ErrorScore,
               "AccuracyScore": AccuracyScore,
               "Edit_error_rate": error_rate,
...
def SelectivePredictEndDecode(error_df, model_dir, error_codewords=None, batch_size=128, num_beams=15):
    ...
    *Apply constrained beam search for prediction decoding*
    ...
    chunks_of_predictions = list2splits(
        [seq.replace(" ", "") for seq in predictions], num_beams)
...
def PredictEvalAll(predictions_sets, model_dir=None, batch_size=128, error_codewords=None, selective_decoding=False, GTGS_As_CL=False):
    ...
    *Predicts and decodes error strings in a data frame*
        ...
        if idx == "GTGS":
            if GTGS_As_CL:
                ...
                if selective_decoding:
                    df, metric = SelectivePredictEndDecode(
                        df, model_name, error_codewords, batch_size=batch_size)
                else:
                    df, metric = PredictEndDecode(
                        df, model_name, error_codewords=None, batch_size=batch_size)
            else:
    ...
    prediction_df = pd.concat(
        [df_preds["SL"], df_preds["LL"], df_preds["CL"], df_preds["GTGS"]])

    subs, dels, ins, error_rate = edit_distance_summ(
        list(prediction_df['orig_DNA']), list(prediction_df['dna_recoverd']))
