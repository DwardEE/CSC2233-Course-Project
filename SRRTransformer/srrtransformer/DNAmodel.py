...
def detailed_edit_distance(string_b, string_a):
    """
    Takes 2 strings and returns the detailed number of edit operations to get from string_a to string_b.
    ...
    Returns:
    --------
    Number of substitutions, number of deletions, number of insertions
    """
...
def edit_distance_summ(column_a=None, column_b=None):
    """
    Given two columns of strings, return the total number of edit operations to get from strings in column_a to column_b.

    Parameters:
    ----------
    :param column_a: df column of strings, Default: None
    :param column_b: df column of strings, Default: None

    Returns:
    --------
    Total number of substitutions, deletions, insertions, and the edit error rate
    """
    substitutions = 0
    deletions = 0
    insertions = 0
    ...
    for string_a, string_b in zip(list(column_a), list(column_b)):
        ...
        substitutions += edit_dist[0]
        deletions += edit_dist[1]
        insertions += edit_dist[2]
...
class DnaModel:
    ...
    def __init__(self, batch_size=32, from_pretrained=False, pretrained_model_path=None, model_dir=None):
        ...
        *Build a DNA model based on the DNA Encoder-Decoder model *
        ...
        safe_factor = 4
        self.encoder_max_length = self.kmer_size * \
            self.orig_sequance_kmer_length - (self.kmer_size-1) + safe_factor
    ...
    def process_data_to_model_inputs(self, batch):
        ...
        *Process the data to prepare for model training*
        ...
        inputs = tokenizer(batch["kmer_W_errors"], padding="max_length", truncation=True,
                           max_length=self.encoder_max_length)  # self.encoder_max_length)
        outputs = tokenizer(batch["split_orig"], padding="max_length", truncation=True,
                            max_length=self.decoder_max_length)  # self.decoder_max_length)
    ...
    def format_data(self, data):
        ...
        *set format of the data to prepare for input to the model*
        ...
        data.set_format(type="torch", columns=[
    ...
    def load_model(self):
        ...
        *load the model*
        ...
        if self.from_pretrained:
            ...
            bert2bert_config = EncoderDecoderConfig.from_json_file(
                "models/encoder_decoder_config/config.json")
    ...
    def compute_metrics(self, pred):
        ...
        subs, dels, ins, error_rate = edit_distance_summ(
            joined_label, joined_pred)

        HammingScore = batch_normalized_hamming_distance(
            joined_pred, joined_label)
    ...
    def train_reconstruction_model(self, train_df, eval_df):
        train_data = self.format_data(Data2Dataset(train_df))
        eval_data = self.format_data(Data2Dataset(eval_df))
