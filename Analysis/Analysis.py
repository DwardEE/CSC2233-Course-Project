...
def plot_oligo_number_distribution(dnas):
    ...
    label = f'{zN}({round(zN/N*100,3)}%) lost' + \
    ...
    plt.text(0.95, 0.95, f'{round(sum(rNs)/len(rNs),2)}',
             fontsize=12,  transform=ax.transAxes, va='top', ha='right')
    plt.text(0.05, 0.95, f'{zN}({round(zN/N*100,3)}%)', fontsize=12,
             color='r',  transform=ax.transAxes, va='top', ha='left')
    return zN
...
def plot_error_distribution(dnas, th=1):
    ...
    ax = sns.distplot(error_nums, hist=False, color="r",
                      kde_kws={"shade": True, 'bw': 0.1})

    plt.text(0.95, 0.95, f'{esn}({round(esn/len(error_nums)*100,2)}%)',
             fontsize=12, color='r', transform=ax.transAxes, va='top', ha='right')
...
def save_simu_result(dnas, file_name='simu_res.dna', ignore_index=None):
    with open(file_name, 'w') as f:
        for i, dna in enumerate(dnas):
            if dna['num'] == 0:
                continue
            if ignore_index and (i in ignore_index):
                continue
...
class dna_chunk:
    ...
    def plot_error_dna_jupyter(self, error, compress=True):
        dna = ''
        prv = 0
        def color(c, s): return '\033[1;' + c + 'm' + s + '\033[0m'
        for e in error:
            pos, typ, base = e
            if compress:
                pos = int(e[0]/2)
    ...
    def plot_voting_result(self):
        ...
        for b in self.BASE:
            ...
            prop = go.Scatter(y=y, marker_color=self.BASE_COLOR[b], mode='markers', marker_size=4, hovertext=[
                              str(p) for p in self.R])
            data.append(prop)

        fig = go.Figure(data)
        fig.update_layout(height=300, width=900, showlegend=False,
                          title="Voting Result", xaxis_title="Position", yaxis_title="Frequency",)
        return fig
